...
class TestOP:
    ...
    def test(self):
        ...
        for ii, cam_file in enumerate(cam_file_list):
            ...
            cam_params = np.load(cam_file, allow_pickle=True, 
            ...
            cam_ext = torch.tensor(cam_params['cam_ext'], dtype=torch.float32, 
                                    device=self.device).unsqueeze(0) #[1,4,4]
            cam_int = torch.tensor(cam_params['cam_int'], dtype=torch.float32, 
                                    device=self.device).unsqueeze(0) # [1,3,3]

            depth, _, max_d = self.data_preprocessing(depth0, 'depth', 
                                target_domain_size=[128, 128]) #[1,1,128,128]
            max_d = max_d.view(1)
            seg, _, _ = self.data_preprocessing(seg0, 'depth', 
            ...
            xhnr_gen = self.model_h.sample(xs_batch,noise_batch_g,noise_batch_l)
...
if __name__ == '__main__':
    ...
    for fileid in range(len(scene_list)):
        ...
        testconfig={
            'outdir': 'results_habitat_stage2_sceneloss/virtualcams/'+scene_list[fileid],
            'ckpt_dir': 'checkpoints/checkpoints_prox_modelS2_batch32_epoch30_LRS0.0003_LRH0.0003_LossVposer0.001_LossKL0.1_LossContact0.001_LossCollision0.01',
