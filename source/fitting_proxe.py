...
class FittingOP:
    ...
    def cal_loss(self, xhr, cam_ext):
        ...
        vid, fid = GeometryTransformer.get_contact_id(body_segments_folder=self.body_segments_folder,
        ...
        loss_contact = self.weight_contact * torch.mean(torch.sqrt(contact_dist+1e-4)/(torch.sqrt(contact_dist+1e-4)+0.01))  
...
if __name__=='__main__':
    ...
    for scenename in scene_test_list:
        fittingconfig={
                    'scene_verts_path': os.path.join(proxe_path, 'scenes_downsampled/'+scenename+'.ply'),
                    'scene_sdf_path': os.path.join(proxe_path, 'scenes_sdf/'+scenename),
                    ...
                    'contact_part': ['back','butt','L_Hand','R_Hand','L_Leg',
                                     'R_Leg','thighs'],
                    'contact_id_folder': os.path.join(proxe_path,'body_segments'),
                    'verbose': False
                }
